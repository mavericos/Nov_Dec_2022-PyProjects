...
class Brick:
    ...
    def interpolate(color_a, color_b, t):
        # 'color_a' amd 'color_b' are RGB tuples
        # 't' is a volue between 0.0 and 1.0
    ...
    for row in range(rows):
        for col in range(cols):
            brick = Brick(col * brick_width + gap * col, row * brick_height +
                          gap * row, brick_width, brick_height, 2, [(0,155,0), (155,0,0)])
            bricks.append(brick)
...
def main():
    ...
    paddle = Paddle(paddle_x, paddle_y, PADDLE_WIDTH, PADDLE_HEIGHT, 'brown')
    ball = Ball(WIDTH/2, paddle_y - BALL_RADIUS, BALL_RADIUS, "brown")
    ...
    def display_text(text):
        ...
        win.blit(text_render, (WIDTH / 2 - text_render.get_width() / 2,
                             HEIGHT / 2 - lost_render.get_height() / 2))
        pygame.display.update()
...
# Работа по
# https://www.youtube.com/watch?v=K-Ek0gT8JPM&list=PLnbmk43t7uQON3m98Biq1UOwt78bDQrZ9&index=68